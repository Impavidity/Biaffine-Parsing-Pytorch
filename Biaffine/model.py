...
class Parser(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_arc_dep = layer.MLP(
            ...
            dropout=0.33)
        nn.init.orthogonal(self.mlp_arc_dep.weight)
        self.mlp_arc_head = layer.MLP(
            in_features = 2*config.lstm_hidden,
            out_features = config.arc_mlp_size,
            activation = nn.LeakyReLU(0.1),
            dropout=0.33)
        nn.init.orthogonal(self.mlp_arc_head.weight)
        self.mlp_rel_dep = layer.MLP(
            in_features = 2*config.lstm_hidden,
            out_features = config.rel_mlp_size,
            activation = nn.LeakyReLU(0.1),
            dropout=0.33)
        nn.init.orthogonal(self.mlp_rel_dep.weight)
        self.mlp_rel_head = layer.MLP(
            in_features = 2*config.lstm_hidden,
            out_features = config.rel_mlp_size,
            activation = nn.LeakyReLU(0.1),
            dropout=0.33)
        nn.init.orthogonal(self.mlp_rel_head.weight)
